...
if [ -z "$DEVTYPE" -o -z "$RAIDTYPE" -o -z "$SCRIPTNAME" ]; then
    ...
    echo "       e.g. sh $0 scsi 1 norm disk_rerr.stp"
    exit
fi
...
#
    ...
    date
    sleep 1
    cat $TESTDIR/targetfile > /tmp/targetfile_after
    sleep 5
    echo "Fault injected"
...
echo "verify the contents when a fault injected(by cmp command)" 
cmp /tmp/$TESTFILENAME /tmp/targetfile_after
echo "cmp result = $?"
